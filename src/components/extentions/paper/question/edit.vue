  ...
  <div class="h-panel w-1200">
    ...
    <div class="h-panel-body">
      <Form mode="block" ref="form" :validOnChange="true" :showErrorTip="true" :rules="rules" :model="question">
        <Row :space="10">
          ...
          <Cell width="6">
            <FormItem label="类型" prop="type">
              <Select v-model="question.type" :disabled="true" :datas="types" keyName="id" titleName="name"></Select>
            </FormItem>
          </Cell>
          <Cell width="6">
        ...
        <FormItem label="习题内容" prop="content" v-if="question.type !== 6">
        ...
        <div class="answer-box">
          ...
          ></paper-question-judge>
          <paper-question-cap :content="JSON.parse(question.content)" @update="contentUpdate" v-else-if="question.type === 6"></paper-question-cap>
        </div>

        <FormItem label="习题解析" prop="remark">
  ...
  methods: {
    ...
    create() {
      ...
      if (validResult.result) {
        R.Extentions.paper.Question.Update(this.question).then(() => {
